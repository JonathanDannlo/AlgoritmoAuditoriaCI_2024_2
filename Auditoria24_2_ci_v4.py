...
def integradorPiam(insumoPiam20242,insumoFacturacion,insumoIcetex):
    filtroInsumoFacturacion = insumoFacturacion[['Documento','Id  factura','Tercero','Aplica gratuidad','Estado Actual',
    ...
    InsumoPiamFC20242['Flag Matricula'] = np.where(InsumoPiamFC20242['MTRNETA'] == InsumoPiamFC20242['Valor Factura'],'Correcta','Errada')
    InsumoPiamFC20242 = InsumoPiamFC20242.sort_values(by='Estado Actual', ascending=True)
    return InsumoPiamFC20242

def verificadorPiam(df):
    validaciones = [
      (df['ESTADO_GIRO'].isin(['Renovado con giro', 'Aprobado con giro']) & df['Relación de Giro'].isnull()),
      (df['ESTADO_GIRO'].isin(['Renovado con giro', 'Aprobado con giro']) & df['Relación de Giro'].notnull()),
      (~df['ESTADO_GIRO'].isin(['Renovado con giro', 'Aprobado con giro']) & df['Relación de Giro'].notnull()),
      (~df['ESTADO_GIRO'].isin(['Renovado con giro', 'Aprobado con giro']) & df['Relación de Giro'].isnull())
    ]
    resultados = [
      'Beneficiario FSE',
      'Beneficiario FSE-ICETEX',
      'Beneficiario ICETEX',
      'Excluido'
    ]
    df['Estado Beneficio'] = np.select(validaciones, resultados, default='')
    return df

def ejecutorPagos(df):
    dfSal = df.copy()
    condicion = (
        (dfSal['Estado Beneficio'] == 'Beneficiario FSE') &
        (dfSal['Estado Actual'] == 'ac') &
        (dfSal['Valor Pagado'] == 39000)
    )
    dfSal['Pago1'] = 0
    dfSal['porcentaje'] = 0
    registrosCondicion = dfSal[condicion]
    numRegistros = registrosCondicion.shape[0]
    if numRegistros > 0:
        montoTotal = 2187068805
        montoPorRegistro = montoTotal / numRegistros
        montoDistribuido = 0
        for index, row in registrosCondicion.iterrows():
            pago = min(math.ceil(montoPorRegistro), row['NETAAPL'])
            dfSal.at[index, 'Pago1'] = pago
            dfSal.at[index, 'porcentaje'] = (pago / row['NETAAPL']) * 100 if row['NETAAPL'] != 0 else 0
            montoDistribuido += pago
        montoRestante = montoTotal - montoDistribuido
        if montoRestante > 0:
            for index, row in registrosCondicion.iterrows():
                maxPago = row['NETAAPL']
                if dfSal.at[index, 'Pago1'] < maxPago:
                    extraPago = min(math.ceil(montoRestante), maxPago - dfSal.at[index, 'Pago1'])
                    dfSal.at[index, 'Pago1'] += extraPago
                    dfSal.at[index, 'porcentaje'] = (dfSal.at[index, 'Pago1'] / maxPago) * 100 if maxPago != 0 else 0
                    montoRestante -= extraPago
                if montoRestante <= 0:
                    break
    return dfSal

def ejecutorReintegros(df):
    dfReintegro = df.copy()
    condicion = (
        (dfReintegro['Estado Beneficio'] == 'Beneficiario FSE') &
        (dfReintegro['Estado Actual'] == 'ca') &
        (dfReintegro['Saldo'] == 0)
    )
    dfReintegro['Reintegro1'] = 0  # Inicializamos la columna con ceros
    dfReintegro.loc[condicion, 'Reintegro1'] = dfReintegro.loc[condicion, 'NETAAPL']
    return dfReintegro
...
try:
    ...
    InsumoPiamFC20242pr = verificadorPiam(InsumoPiamFC20242)
    InsumoPiamFC20242prp = ejecutorPagos(InsumoPiamFC20242pr)
    InsumoPiamFC20242prpr = ejecutorReintegros(InsumoPiamFC20242prp)
...
with pd.ExcelWriter(output_pathXlsx, engine='xlsxwriter') as writer:
    ...
    if InsumoPiamFC20242prp is not None:
      InsumoPiamFC20242prp.to_excel(writer, sheet_name='PIAMFC20242', index=False)
      print('Se ha generado la plantilla del PIAMF 2024-2')"""
    if InsumoPiamFC20242prpr is not None:
      InsumoPiamFC20242prpr.to_excel(writer, sheet_name='PIAMFC20242PR', index=False)
